...
int main()
{
    ...
    for( int n=1; n<=N; n++)
    {
        for( int m=1; m <= M; m++)
        {
            if(ingrid[n-1][m] >= 0 && n > 1)
                v[n][m].push_back( make_pair(n-1, m) );

            if(ingrid[n-1][m+1] >= 0 && n > 1 && m < M)
                v[n][m].push_back( make_pair(n-1, m+1) );

            if(ingrid[n][m+1] >= 0 && m < M)
                v[n][m].push_back( make_pair(n, m+1) );

            if(ingrid[n+1][m] >= 0 && n < N)
                v[n][m].push_back( make_pair(n+1, m) );

            if(ingrid[n+1][m-1] >= 0 && m > 1 && n < N )
                v[n][m].push_back( make_pair(n+1, m-1) );

            if(ingrid[n][m-1] >= 0 && m > 1)
                v[n][m].push_back( make_pair(n, m-1) );
        }
